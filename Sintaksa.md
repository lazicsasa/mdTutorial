...
```php
  class User extends DataBaseCredentials {
    // Podaci za upis novog korisnika u bazu
      protected $table = "korisnik";
      public $name = "";
      public $email = "";
      public $tp_02 = "TP_02";

    // Konstruktor klase - Proverava se postojanje baze. Ako ne postoji kreira se baza, tako i sa tabelom u bazi
      public function __construct($name, $email) {
        // 1. Pokušava se konekcija na server bez kreiranja databaze
          $this->conn = new mysqli($this->host, $this->username, $this->password);
          if ($this->conn->connect_error) {
              die("Connection failed: " . $this->conn->connect_error);
          }

        // 2. Kreira se SQL Baza ako ne postoji
        ...
        // 3. Izbor SQL Baze za manipulaciju podacima
          $this->conn->select_db($this->dbname);
          $this->conn->set_charset('utf8mb4'); // Dobra praksa je da se setuje charset u konekciji sa bazom kako bi se pravilno ispisivala srpska slova

        // 4. Kreiranje tabele ako ne postoji
          $sql = "
        ...
        // 6. Zatvaranje konekcije sa bazom
          $this->conn->close();
      }
  }
```
